...
- **3.1.3. '관심 있어요' 표현 및 채팅방 개설:**
  ...
  - **알림(헤더 드롭다운):** 상단 우측의 알림 아이콘 버튼을 클릭하면 최근 알림 일부가 드롭다운으로 노출됩니다.
    - 초기 지원 알림 종류(2종):
      1. **내 포스트에 '관심 있어요' 발생 알림**(포스트 작성자에게 도달)
      2. **채팅방 생성 알림**(해당 채팅방의 모든 멤버에게 도달)
    - 각 알림 항목은 제목/간단한 설명/시간을 포함하며 클릭 시 관련 페이지로 이동합니다(예: 포스트 상세, 채팅방).
...
7. **내 활동 보기:** 내가 만든 포스트/관심 표현한 포스트로 이동하여 관리.
...
- **핵심 요소:**
  ...
  - **헤더 우측 아이콘:**
    - **프로필 버튼:** 클릭 시 `/profile`로 이동
    - **알림 버튼:** 클릭 시 최근 알림 드롭다운 오픈(초기 2종 알림 노출)
  - **하단 네비게이션:** 홈, 채팅방, 내 활동. (채팅방, 내 활동은 로그인 필요 시 로그인 유도)
- **인터랙션:**
...
**10. 내 활동 화면 (`/my-activities`)**

- **목적:** 내 활동들을 확인하고 관리.
- **핵심 요소:**
  - **상단:** 뒤로 가기 버튼, "내 활동" 제목.
  - **내가 만든 포스트 목록(중앙 영역의 상단):** 스크롤 가능한 카드 형태.
    - 카드 내용: 포스트 이미지(선택), 제목, 생성일, 상태(대기/매칭/만료), 관심 있어요 수.
    - 필요 시 무한 스크롤.
  - **관심 있어요 누른 목록(중앙 영역의 하단):** 스크롤 가능한 카드 형태.
    - 카드 내용: 포스트 이미지(선택), 제목, 생성일, 상태(대기/매칭/만료), 관심 있어요 수.
    - 필요 시 무한 스크롤.
